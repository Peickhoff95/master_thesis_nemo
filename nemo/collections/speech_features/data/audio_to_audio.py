...
class AudioToAudioWhisperDataset(Dataset):
    """
    Dataset that loads tensors via a json file containing paths to audio files and durations (in seconds).
    Each new line is a different sample. Example below:
    {"audio_filepath": "/path/to/audio.wav", "duration": 23.147}
    ...
    def __getitem__(self, index):
        ...
        features_input_tmp = features_input[None, :]
        fl_tmp = torch.tensor(features_input_tmp.shape[1]).long()[None]

        features_input = whisper.pad_or_trim(features_input)
        ...
        features_target = whisper.pad_or_trim(features_target)
        ...
        fl = fl[None]
        if self.preprocessor is not None:
            _, processed_signal_length = self.preprocessor(input_signal=features_input_tmp, length=fl_tmp)
            processed_input_signal, _ = self.preprocessor(
                input_signal=fi, length=fl,
            )
            processed_target_signal = whisper.log_mel_spectrogram(
                features_target
            )
            fi, ft, fl = processed_input_signal[0], processed_target_signal, processed_signal_length[0]
