...
def get_whisper_dataset(
        config: dict, tokenizer: 'TokenizerSpec', augmentor: Optional['AudioAugmentor'] = None, preprocessor: Optional['AudioPreprocessor'] = None
) -> audio_to_text.AudioToBPEDataset:
    """
    Instantiates a Byte Pair Encoding / Word Piece Encoding based AudioToBPEDataset.
    ...
    Returns:
        An instance of AudioToBPEDataset.
    """
    dataset = audio_to_text.AudioToBPEWhisperDataset(
        manifest_filepath=config['manifest_filepath'],
        tokenizer=tokenizer,
        sample_rate=config['sample_rate'],
        int_values=config.get('int_values', False),
