...
def get_char_dataset(config: dict, augmentor: Optional['AudioAugmentor'] = None) -> spectogramm_to_text.SpectogrammToCharDataset:
    """
    ...
    dataset = spectogramm_to_text.SpectogrammToCharDataset(
...
def get_bpe_dataset(
    ...
    """
    dataset = spectogramm_to_text.SpectogrammToBPEDataset(
        manifest_filepath=config['manifest_filepath'],
        tokenizer=tokenizer,
        sample_rate=config['sample_rate'],
        int_values=config.get('int_values', False),
...
#    ) -> Union[spectogramm_to_text.TarredAudioToBPEDataset, spectogramm_to_text.TarredAudioToCharDataset]:
#        """
...
#                dataset = spectogramm_to_text.TarredAudioToCharDataset(
...
#                )
#            else:
#                dataset = spectogramm_to_text.TarredAudioToBPEDataset(
#                    audio_tar_filepaths=tarred_audio_filepath,
#                    manifest_filepath=manifest_filepath,
#                    tokenizer=tokenizer,
#                    sample_rate=config['sample_rate'],
...
#    ) -> spectogramm_to_text_dali.AudioToCharDALIDataset:
#        """
...
#        dataset = spectogramm_to_text_dali.AudioToCharDALIDataset(
#            manifest_filepath=config['manifest_filepath'],
#            device=device,
...
#        """
#        device = 'gpu' if torch.cuda.is_available() else 'cpu'
#        dataset = spectogramm_to_text_dali.AudioToBPEDALIDataset(
#            manifest_filepath=config['manifest_filepath'],
#            tokenizer=tokenizer,
#            device=device,
#            batch_size=config['batch_size'],
...
#                    datasets[idx] = spectogramm_to_text.BucketingDataset(
...
#            return spectogramm_to_text.RandomizedChainDataset(datasets=datasets, rnd_seed=0)
#        elif bucketing_strategy == 'fully_randomized':
#            return spectogramm_to_text.RandomizedChainDataset(datasets=datasets, rnd_seed=random.randint(0, 30000))
#        else:
#            raise ValueError(
