...
class ReconstructionModel(ExportableEncDecModel, ModelPT, ReconstructionMixin, ASRBPEMixin):
    ...
    def _setup_dataloader_from_config(self, config: Optional[Dict]):
        ...
        if config.get('is_tarred', False):
            ...
            dataset = audio_to_audio_dataset.get_audio_dataset(
                config=config,
...
class ReconstructionASRModel(ReconstructionModel):
    @classmethod
    def list_available_models(cls) -> Optional[PretrainedModelInfo]:
        """
        This method returns a list of pre-trained model which can be instantiated directly from NVIDIA's NGC cloud.
    ...
    def __init__(self, cfg: DictConfig, trainer: Trainer = None):
        ...
        self.loss = CTCLoss(
            num_classes=self.ASRModel.decoder.num_classes_with_blank - 1,
            zero_infinity=True,
            reduction=self._cfg.get("ctc_reduction", "mean_batch"),
        )

        if hasattr(self._cfg, 'load_ASR_weights') and self._cfg.load_ASR_weights is not None:
            self.ASRModel.load_state_dict(
                torch.load(
                    self._cfg.load_ASR_weights,
                    map_location=torch.device(self.device)),
                strict=True,
            )
    ...
    def forward(
        ...
        encoded, encoded_len = self.encoder(audio_signal=processed_signal, length=input_signal_length)
        ...
        return log_probs, encoded_len, greedy_predictions, log_spec

    # PTL-specific methods
    def training_step(self, batch, batch_nb):
        signal, target, signal_len, transcript, transcript_len = batch
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        signal, _, signal_len, transcript, transcript_len = batch
        log_probs, encoded_len, predictions, _ = self.forward(input_signal=signal, input_signal_length=signal_len)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        signal, target, signal_len, transcript, transcript_len = batch
        log_probs, encoded_len, predictions, log_spec = self.forward(input_signal=signal, input_signal_length=signal_len)

        if target.shape[2] != log_spec.shape[2]:
            target = torch.nn.functional.pad(target, (0, log_spec.shape[2] - target.shape[2]), 'constant', 0.0)
        ...
        plt.subplot(3, 1, 2)
        plt.title('Prediction')
        plt.pcolormesh(log_spec[0].cpu().detach().numpy(), vmin=vmin, vmax=vmax)
        plt.colorbar()
        plt.subplot(3, 1, 3)
        plt.title('Clean')
        plt.pcolormesh(target[0].cpu().detach().numpy(), vmin=vmin, vmax=vmax)
    ...
    def multi_validation_epoch_end(self, outputs, dataloader_idx: int = 0):
        ...
        tensorboard_logs = {'val_loss': val_loss_mean}
        # Log spectograms
        self.trainer.logger.experiment.add_image('spectograms', np.stack(specs), global_step=self.global_step,
                                                 dataformats='NCHW')
        # Log weighted sum
        plt.switch_backend('agg')
        fig_wsum = plt.figure()
        plt.bar(range(self.encoder.weighted_sum.weight.shape[1]),
                self.encoder.weighted_sum.weight[0].cpu().detach().numpy())
        self.trainer.logger.experiment.add_image('weighted_sum/weights_bar', figure_to_image(fig_wsum, close=True),
                                                 global_step=self.global_step)
        self.trainer.logger.experiment.add_histogram('weighted_sum/weights', self.encoder.weighted_sum.weight,
                                                     global_step=self.global_step)
        if self._cfg.encoder.wsum_bias:
            self.trainer.logger.experiment.add_histogram('weighted_sum/bias', self.encoder.weighted_sum.bias,
                                                         global_step=self.global_step)
        return {'val_loss': val_loss_mean, 'log': tensorboard_logs}
